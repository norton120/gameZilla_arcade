...
create: function(){
	...
	var menuBody = game.add.tileSprite(game.world.width/4, game.world.height/6, game.world.width/2, game.world.height/1.5, 'blackFill');
	var gameLogo = game.add.image((menuBody.x + Math.floor(menuBody.x*.2)), (menuBody.y + Math.floor(menuBody.y*.15)) , 'revzillaLogo');	

	var logoScale = (menuBody.width*.8)/gameLogo.width;
	gameLogo.scale.setTo(logoScale,logoScale);
	var Subtitle = game.add.text(0, gameLogo.y + gameLogo.height + 25, 'The Battle For Philadelphia', {font: ' 2rem Press Start 2P', fill: '#fefefe', });
	...
	game.time.events.loop(500,game.checkForCoins,this);
       ...
       //listen for the fake coin drop ('c' key)
       //game.fakeCoin.onDown.add(function(){game.coinCredit++});
},
